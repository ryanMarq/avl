...
class example_env(avl.Env):
    def __init__(self, name, parent):
        ...
        # Get variable env.c which was set to 100 and then overriden to 300
        self.c = avl.Factory.get_variable(f"env.c")
        if self.c != 300:
            self.error("Expected c to be 300")
...
async def test(dut):
    ...
    # This should not override a because the "override" flag is not set
    avl.Factory.set_variable("env.a", 200) 

    # Initially, set c to 100
    avl.Factory.set_variable("env.c", 100)

    # Override the value of c to 300
    avl.Factory.set_variable("env.c", 300, override=True)
